...
static func Cast(agent : BaseAgent, target : BaseAgent, skill : SkillCell):
	...
	if SkillCommons.TryConsume(agent, SkillCommons.ConsomeType.MANA, skill):
		Stopped(agent)
		agent.SetSkillCastName(skill.name)
...
static func Damaged(agent : BaseAgent, target : BaseAgent, skill : SkillCell, rng : float):
	...
	Launcher.Network.Server.NotifyInstance(agent, "TargetAlteration", [target.get_rid().get_id(), info.value, info.type, skill.name])
...
static func Healed(agent : BaseAgent, target : BaseAgent, skill : SkillCell, rng : float):
	...
	Launcher.Network.Server.NotifyInstance(agent, "TargetAlteration", [target.get_rid().get_id(), heal, ActorCommons.Alteration.HEAL, skill.name])
...
static func Delayed(agent : BaseAgent, target : BaseAgent, skill : SkillCell):
	Launcher.Network.Server.NotifyInstance(agent, "TargetAlteration", [target.get_rid().get_id(), 0, ActorCommons.Alteration.PROJECTILE, skill.name])
